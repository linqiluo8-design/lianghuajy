...
> ## 🔥 最高准则（不可违背）⭐⭐⭐⭐⭐⭐
>
> **所有数据必须立即处理：立即拉取、立即更新、立即聚合、立即统计、立即展示**
>
> - ✅ **立即拉取**：采集间隔 1-2 秒，不得超过 3 秒
> - ✅ **立即更新**：数据到达后立即写入数据库，无缓存延迟
> - ✅ **立即聚合**：数据保存后立即触发聚合，不等待下一个周期
> - ✅ **立即统计**：聚合完成后立即更新统计表，无批处理延迟
> - ✅ **立即展示**：统计数据更新后前端自动刷新，用户看到最新数据
> - ❌ **禁止**：任何形式的延迟处理、批量处理、定时任务
>
> **原因**：在股市中，1 秒的延迟可能意味着错失涨停板机会。系统必须在最短时间内处理和展示数据。

## 🎯 核心原则

1. **极致实时性** ⭐⭐⭐⭐⭐
   - 采集间隔：1-2 秒（非 15 分钟）
   - 数据延迟：< 3 秒（含采集、保存、聚合、统计）
   - 每次采集后立即保存、立即聚合、立即统计
...
### 0. 极致实时性原则 ⭐⭐⭐⭐⭐
**遵循最高准则：立即拉取、立即更新、立即聚合、立即统计、立即展示**

> "在股市中，1秒的延迟可能意味着错失涨停板机会。系统必须在最短时间内处理和展示数据，让交易者捕捉到最快的市场变化。"

**数据处理流程（强制执行）**：
```
AkShare API 拉取 (1-2秒间隔)
    ↓ 立即（< 0.1秒）
写入 daily_limit_stats
    ↓ 立即（< 0.1秒）
触发 sector_aggregator.aggregate()
    ├─ 聚合 sector_daily_stats
    └─ 聚合 limit_reason_stats
    ↓ 立即（< 0.5秒）
前端自动刷新展示
```
...
### 🔄 数据源优先级（聚合逻辑）
**板块数据聚合优先级**：
1. **优先级1：概念板块** (`concept_tags`) - 当有概念数据时使用
   - 一只股票可属于多个概念（通过`;`分隔）
   - 使用 `UNNEST(STRING_TO_ARRAY())` 拆分聚合

2. **优先级2：所属行业** (`industry`) - 当概念为空时使用 ⭐
   - AkShare 强势池 API 提供的行业字段
   - 当前实现：使用此字段进行板块分类
   - 示例：电子、计算机、通信、医药生物等

3. **优先级3：板块映射** (`stock_sector_mapping`) - 历史数据或手动维护
   - 需要预先建立股票与板块的映射关系
...
**当前状态**（2026-01-05 v2）：
- ✅ 使用优先级2：按所属行业聚合
- ❌ concept_tags 为空（AkShare免费API限制）
- ✅ industry 字段有数据（基础池+强势池合并）
- 📊 数据完整性：基础池所有涨停（108只）+ 强势池行业信息
- 📊 聚合结果：多个行业板块（电子、计算机、通信等）

**数据采集策略**（2026-01-05更新）：
```python
# 双API合并策略
基础涨停池 (stock_zt_pool_em)     ← 所有涨停股票（108只）
   +
强势涨停池 (stock_zt_pool_strong_em) ← 行业、原因字段（88只）
   =
完整数据：108只股票 + 88只有行业信息
```

**字段来源**：
- 基础字段（代码、名称、价格等）：来自基础池
- 行业字段（所属行业、入选理由）：来自强势池（通过股票代码匹配）
- 未匹配股票：行业字段为空，但仍会显示在板块统计中
